...
export const IosPickerItem = (props) => {
  const { names, perspective, loop = false } = props; 
  ...
  const totalRadius = names.length * WHEEL_ITEM_RADIUS; 
  ...
  const rotateWheel = useCallback(
    (emblaApi) => {
      const rotation = names.length * WHEEL_ITEM_RADIUS - rotationOffset; 
      const wheelRotation = rotation * emblaApi.scrollProgress();
      setContainerStyles(emblaApi, wheelRotation); 
      emblaApi.slideNodes().forEach((_, index) => {
        setSlideStyles(emblaApi, index, loop, names.length, totalRadius);
      });
    },
    [names.length, rotationOffset, totalRadius, loop]
  );
